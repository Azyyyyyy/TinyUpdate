...
{
    private static readonly ILogger Logging = LogManager.CreateLogger(nameof(ReleaseFile));
    ...
    public static async Task<bool> CreateReleaseFile(IEnumerable<ReleaseFile> releaseFiles, string fileLocation)
    {
        ...
        {
            Logging.Log(Level.Error, $"Directory {fileLocation} doesn't exist, failing...");
            return false;
        }
        ...
        if (file.Exists)
        {
            Logging.Log(Level.Warn, $"{fileLocation} already exists, grabbing all valid releases and recreating file");
    ...
    public static ReleaseFile? ReadReleaseFile(string line, out bool successful)
    {
        ...
        Logging.Log(Level.Warn, $"Line {line} is not a valid ReleaseFile");
        return null;
    }
}