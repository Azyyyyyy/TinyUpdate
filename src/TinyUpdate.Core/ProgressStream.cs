...
public class ProgressStream : Stream
{
    ...
    private readonly ILogger _logger = LogManager.CreateLogger(nameof(ProgressStream));
    public override async Task CopyToAsync(Stream destination, int bufferSize, CancellationToken cancellationToken)
    {
        ...
        if (amountToWrite == -1)
        {
            _logger.Warn("We can't give a report for CopyToAsync due to one of the streams not supporting Seeking");
