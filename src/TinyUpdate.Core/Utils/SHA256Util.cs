...
{
    private static readonly ILogger Logger = LogManager.CreateLogger(nameof(SHA256Util));
    ...
    public static bool CheckSHA256(byte[] byteArray, string expectedSHA256)
    {
        if (IsValidSHA256(expectedSHA256))
        {
            ...
            Logger.Log(Level.Info, $"Do we have the expected SHA256 hash?: {sameHash}");
            return sameHash;
        }

        Logger.Warn("We been given an invalid hash, can't check");
        return false;
    }
