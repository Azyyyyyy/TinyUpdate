...
{
    private static readonly ILogger Logger = LogManager.CreateLogger(nameof(TemporaryFolder));
    ...
    public void Dispose()
    {
        ...
        {
            Logger.Log(Level.Warn, $"{Location} still contains some files, these will be deleted!");
        }
        ...
        {
            Logger.Log(Level.Warn, $"Something happened while trying to cleanup {Location}, this folder might still exist and need manual cleanup!");
            Logger.Log(e);
        }
    }
}