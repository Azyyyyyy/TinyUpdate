...
namespace System.Net.Http
{
    public static class HttpClientFactory
    {
        ...
        public static HttpMessageHandler CreatePipeline(HttpMessageHandler innerHandler, IEnumerable<DelegatingHandler>? handlers)
        {
            ...
            {
                throw new Exception("We need inner handler");
            }
            ...
            foreach (DelegatingHandler handler in reversedHandlers)
            {
                if (handler == null)
                {
                    throw new Exception(); // Error.Argument("handlers", Properties.Resources.DelegatingHandlerArrayContainsNullItem, nameof(DelegatingHandler));
                }

                if (handler.InnerHandler != null)
                {
                    throw new Exception(); //Error.Argument("handlers", Properties.Resources.DelegatingHandlerArrayHasNonNullInnerHandler, nameof(DelegatingHandler), "InnerHandler", handler.GetType().Name);
                }
